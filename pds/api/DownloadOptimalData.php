...
if (isset($_GET['format'])) {
    ...
    $columns = ["scenario","from","from_state","from_id","from_name","from_district","from_lat","from_long","to","to_state","to_id","to_name","to_district","to_lat","to_long","commodity","quantity","distance","approve","reason","new_id","new_distance"];
    $month = $_GET['month'];
    ...
    if($numrows>0){
        while($row = mysqli_fetch_array($result)){
			if($row['new_id_admin']!=null or $row['new_id_admin']!=""){
				...
				$row["from_name"] = $row['new_name'];
				$row["distance"] = $row['new_distance'];
				echo "hi";
			}
            $temp = array();
    ...
    echo 'Please specify a format in the GET request (e.g., ?format=pdf).';
}
