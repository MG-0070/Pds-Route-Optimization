					...
					success: function(result){
						...
						try {
							...
							for (var ids in warehousearray) {
								...
								warehousepart += "<option value=" + warehouse_id + ">" + warehouse_id + "</option>";
							}
							...
							for (var datafield in obj) {
								...
								var reviewpart = "";
								if (approved === "yes") {
									reviewpart = "<td><button class='btn btn-info'>Already Reviewed</button></td><td>" + reason + "</td>";
								} else if (approved === "no") {
									reviewpart = "<td><button class='btn btn-danger'>Not Approved</button></td><td>" + reason + "</td>";
								} else {
									reviewpart = "<td><select class='form-control' onchange='enableDisable(\"" + uniqueid + "\")' id='" + uniqueid_bool + "' name='" + uniqueid_bool + "' required><option value=''>Select</option><option value='yes'>Approve</option><option value='no'>Change ID</option></select></td><td><select class='form-control' onchange='handleReasonChange(\"" + uniqueid_idreason + "\")' id='" + uniqueid_idreason + "' name='" + uniqueid_idreason + "' disabled><option value=''>Select</option><option value='Road not accessible'>Road not accessible</option><option value='Road repair going on'>Road repair going on</option><option value='Pertaining to Distance'>Pertaining to Distance</option></select></td>";
								}

								var newdistance = new_distance ? "<td>" + new_distance + "</td>" : "<td><input type='text' onchange='handleDistanceChange(\"" + uniqueid_iddistance + "\")' id='" + uniqueid_iddistance + "' name='" + uniqueid_iddistance + "' disabled required /></td>";
								var newidadminTd = newidadmin ? "<td>" + newidadmin + "</td>" : "<td><select class='form-control' onchange='handleNewIdChange(\"" + uniqueid + "\")' id='" + uniqueid + "' name='" + uniqueid + "' disabled required><option value=''>Select Id</option>" + warehousepart + "</select></td>";

								subpart1 += reviewpart + newidadminTd + newdistance + "</tr>";
								tableBody += subpart1;
							}
