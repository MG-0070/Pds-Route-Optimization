...
def processFile():
    ...
    #    Variables for Wheat from IG TO FPS
       ...
       # Calling CBC_CMB Solver
        ...
        #df51['From_ID'] = df51['From_ID'].combine_first(df51['From_ID'].astype(str))
        #df51['To_ID'] = pd.to_numeric(df51['To_ID'], errors='coerce')
        ...
        df5['To_ID'] = pd.to_numeric(df5['To_ID'], errors='coerce').fillna(df5['To_ID'])
        ...
        data["WH_Used_Baseline"] = "255"
        
        data["FPS_Used"] = df5['To_ID'].nunique()
        data["FPS_Used_Baseline"] = "17,829"
        
        data['Demand'] = int(FPS['Allocation_Wheat'].sum())
        data['Demand_Baseline'] = "87,13,290"
        
        data['Total_QKM'] = float(result)
        data['Total_QKM_Baseline'] = "11,58,22,464"
        
        data['Average_Distance'] = float(round(result, 2)) / Total_Demand
        data['Average_Distance_Baseline'] = "13.29"
        
        wheat_total1=int(FPS['Allocation_Wheat'].sum())
        ...
        file_open.write('participant "FCI\\n<size:40><&globe>" as FCI order  1 \n')
        file_open.write('participant "FPS\\n<size:40><&vertical-align-top>" as FPS order 2 \n')
        file_open.write('FCI -[#32a8a0]> FPS: <font color=#0915ed> ' + str(wheat_total1) + ' \n')
        file_open.write('hnote over FCI, FPS #ffffff: ' + str(wheat_total1) + ',' + ' Qtl Wheat \n')
        file_open.close()
...
def cleanup():
    ...
    # Release memory allocated by the Python interpreter
    gc.set_threshold(0)

    # Release any resources that might have been opened
