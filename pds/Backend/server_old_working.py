...
def fci_data():
    try:
        ...
        districts_no = fps['FPS_District'].nunique()
...
def save_to_database(month, year, applicable):
    ...
    if connection.is_connected():
        ...
        cursor.execute(sql)
        sql = "CREATE TABLE " + table_name + " ( from_district VARCHAR(150) NOT NULL, from_id VARCHAR(150) NOT NULL, from_name VARCHAR(150) NOT NULL, to_district VARCHAR(150) NOT NULL, to_id VARCHAR(150) NOT NULL, to_name VARCHAR(150) NOT NULL, approve VARCHAR(100) , new_id VARCHAR(100) , new_id_admin VARCHAR(100), old_id VARCHAR(100) , new_name VARCHAR(255) , reviewed VARCHAR(10) )";
        cursor.execute(sql)
        for (index, row) in selected_data.iterrows():
            sql = 'INSERT INTO ' + table_name + ' (from_district, from_id, from_name, to_district, to_id, to_name) VALUES (%s, %s, %s, %s, %s, %s)'
            values = tuple(row)
...
def processFile():
    ...
    #    Variables for Wheat from IG TO FPS
       ...
       # Calling CBC_CMB Solver
        ...
        #df10 = df9.append([df7],ignore_index=True)
        result = (df10['Values'] * df10['Distance']).sum()
        print(result)
